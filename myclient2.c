...
int performUselessAction() {
    static int uselessCounter = 0;
    uselessCounter += rand() % 100;  // 랜덤한 숫자 더하기
    printf("정말 쓸데없는 행동: 현재 카운터 값은 %d\n", uselessCounter);
    return uselessCounter;
}

// 아주 쓸데없는 행동을 하는 함수 (랜덤 문자열 생성 후 출력)
void printRandomString(char *randomString, size_t length) {
    ...
    printf("정말 쓸데없는 랜덤 문자열: %s\n", randomString);
}

// 서버에 값을 쓰는 함수
UA_StatusCode writeValueToServer(UA_Client *client, UA_NodeId nodeId, UA_Variant *myVariant) {
    return UA_Client_writeValueAttribute(client, nodeId, myVariant);
}
...
int main(int argc, char *argv[]) {
    ...
    UA_StatusCode retval = UA_Client_getEndpoints(client, "opc.tcp://192.168.200.63:4840",
    ...
    retval = UA_Client_connectUsername(client, "opc.tcp://192.168.200.63:4840", "user1", "password");
    if(retval != UA_STATUSCODE_GOOD) {
...
while (true) {
        // 1. 쓸데없는 숫자 작업을 하고 서버에 쓰기
        int uselessValue = performUselessAction();
        UA_Variant *myVariant = UA_Variant_new();
        UA_Variant_setScalarCopy(myVariant, &uselessValue, &UA_TYPES[UA_TYPES_INT32]);
        
        // 'the.useless.value'라는 노드에 값을 씁니다 (서버에 미리 정의된 노드라고 가정)
        UA_NodeId nodeId = UA_NODEID_STRING(1, "the.useless.value");
        retval = writeValueToServer(client, nodeId, myVariant);
        if (retval == UA_STATUSCODE_GOOD)
            printf("서버에 쓸데없는 숫자 값 %d 전송 성공\n", uselessValue);
        else
            printf("서버에 값 전송 실패\n");
        UA_Variant_delete(myVariant);

        // 2. 쓸데없는 랜덤 문자열을 만들고 서버에 쓰기
        char randomString[6];  // 5자리 문자열 + null terminator
        printRandomString(randomString, sizeof(randomString));

        myVariant = UA_Variant_new();
        UA_String uaString = UA_STRING(randomString);
        UA_Variant_setScalarCopy(myVariant, &uaString, &UA_TYPES[UA_TYPES_STRING]);
...
// 임시 UA_NodeId 변수를 생성합니다.
UA_NodeId tempNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER);
UA_NodeId_copy(&tempNodeId, &bReq.nodesToBrowse[0].nodeId);
...
// 나머지 코드...
            ...
            for(size_t i = 0; i < bResp.resultsSize; ++i) {
                for(size_t j = 0; j < bResp.results[i].referencesSize; ++j) {
                    ...
                    if(ref->nodeId.nodeId.identifierType == UA_NODEIDTYPE_STRING) {
                        // 노드가 존재하면 NodeId를 설정합니다.
                        nodeId = UA_NODEID_STRING(1, (char*)ref->nodeId.nodeId.identifier.string.data);
                        break;
                    }
                }
            }
        ...
        if (retval == UA_STATUSCODE_GOOD)
            printf("서버에 쓸데없는 문자열 '%s' 전송 성공\n", randomString);
        else
            printf("서버에 문자열 전송 실패\n");
        UA_Variant_delete(myVariant);

        // 주기적으로 서버와 통신 유지
        UA_Client_run_iterate(client, 1000);  // 1초 대기
}
